...
router.post("/api/products", passport.authenticate("current", { session: false }), checkRole(["admin", "premium"]), ProductController.saveProduct); // crear un producto
...
router.delete("/products/:id", passport.authenticate("current", { session: false }), checkRole(["admin", "premium"]), ProductController.deleteProduct); // eliminar un producto especifico por id 
